...
- `crossOrigin.urlPattern`: (`string`) change url to target a cors proxy, from a webcontainer (>= 0.1.18)
- `crossOrigin.whitelist` (`string[]`) domain names used in a webcontainer that should not go through cors proxy (>= 0.1.18)
- `crossOrigin.serverSide` (`boolean`) activates the CORS proxy even while running as a server (>= 0.1.25)
